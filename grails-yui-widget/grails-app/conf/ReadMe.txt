OVERALL TASKS
1. Minimize copied code.
2. Allow HTML elements to be passed in (instead of plugin creating them)
3. Reskinning
4. Resource TagLib needs js/css inclusion capability
5. Grails YUI Plugin - upgrade to 2.9.0
6. Documentation
7. JIRA / Mailing List
8. Test with previous verions of Grails
9. Static Name Prefixes
    - "grailsYuiCalendar_", "grailsYuiDialog_"
    - static availability in both javascript and java
10. Static Namespace

Extensibility
1. Needs to be extendable / customizable
    1.1 Examples of extending / overriding Util obj

DataTable Tasks
3. XML over XHR
4. XML over XHR with Paging
5. Expanding Row
8. XML over XHR with Sorting
9. XML over XHR with Sorting & Sorting
10. Single Select
11. MultiSelect

Dialog
4. Loading Dialog


Plans for next version
1. Allow HTML elements to be passed as part of config - instead of plugin creating it
2. Expandible tables
3. Static Name Prefixes (Grails)
4. Static Namespace (Grails)
5. Static Name Prefixes (Javascript)
6. Static Namespace (Javascript)
7. Create JIRA for this project : http://jira.grails.org/browse/ADMIN
8. Single Select DataTable
9. Multi Select DataTable

Plans for subsequent version
1. Update Grails Yui Plugin to 2.9.0
2. Inclusion of Resources Plugin
3. Skin Example App
4. Post Example App on GAE



Version 0.1.3
- Pending

Version 0.1.2 - RELEASED
- Removed JSON Source - Added JSON JAR

Version 0.1.1 - RELEASED
- Added Calendar support

Version 0.1.0 - RELEASED
- Initial release





